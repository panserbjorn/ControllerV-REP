...
def generateNewSec(secuence, numberOfSec):
	...
	while len(newSecuences) < numberOfSec:
		...
		for instruct in secuence:
			movement = instruct[0]
			time = instruct[1]
			timeModification = np.random.randint(-5,5)
			movementmodification = np.random.randint(-5,5)
			nextSec.append((abs((movement+movementmodification)%10),abs(time+timeModification)))
