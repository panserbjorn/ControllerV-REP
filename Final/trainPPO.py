...
import scipy.signal
import argparse
from mySimulatedEnv import mySimulatedEnv
...
def run_policy(env, policy, scaler, logger, numEpisodes):
	...
	logger.log({'_MeanReward': np.mean([t['rewards'].sum() for t in trejctories]),
				'Steps': total_steps})
	return trajectories
...
def discount(x, gamma):
	...
	'''
	return scipy.sgnal.lfilter([1.0],[1.0, - gamma], x[::-1])[::-1]
